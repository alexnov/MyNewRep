...
public class CementsVolume {
	...
	private static double calcWellVolume(double top, double bottom) {
		// Проверяем, находится ли верх цементного раствора выше башмака
		// предыдущей колонны
		int shoeindex = Geometry.lastPreviousElement();
		double shoe = Program.previous[shoeindex][1]; // глубина по стволу
														// башмака предыдущей
														// колонны
		if (top > shoe) {
			return calcOpenHoleVolume(top, bottom);
		}
		// Проверяем, не находится ли весь раствор в межколонном пространстве
		if (bottom < shoe) {
			return calcPreviousVolume(top, bottom);
		} else {
			return calcOpenHoleVolume(shoe, bottom)
					+ calcPreviousVolume(top, shoe);
		}
	}
	...
	private static double calcCasing(double height) {
		...
		while (i <= Geometry.lastCasingElement()) {
			if (height >= Program.casing[i][1]) {
				casing = casing + calcCasingUnit(i);
			} else {
				casing = casing + calcPartCasingUnit(i, height);
				break;
			}
			i++;
		}
	...
	private static double calcOpenHole(double height) {
		...
		while (i <= Geometry.lastOpenholeElement()) {
			if (height >= Program.openhole[i][1]) {
				openhole = openhole + calcOpenUnit(i);
			} else {
				openhole = openhole + calcPartOpenUnit(i, height);
				break;
			}
			i++;
		}
	...
	// Расчет объема скважины в межколонном пространстве от устья до заданной
	// глубины
	private static double calcPrevious(double depth) {
		double vol = 0.0;
		int i = 0;
		while (i <= Geometry.lastPreviousElement()) {
			if (depth >= Program.previous[i][1]) {
				vol = vol + calcPreviousUnit(i);
			} else {
				vol = vol + calcPartPreviousUnit(i, depth);
				break;
			}
			i++;
		}
