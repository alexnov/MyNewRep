...
public class MainWindow extends JFrame {
	...
	// Обработка нажатия клавиши + - добавляем единицу к счетчику и обновляем
	// таблицу профиля
	...
	private void minusProfileActionPerformed(ActionEvent e) {
		...
		if (index > 1) {
			...
			JOptionPane.showMessageDialog(scroll1,
					"Первую строку нельзя удалить", "Ошибка",
	...
	private void plusSpacersActionPerformed(ActionEvent e) {
		if (index6 > 4) {
			JOptionPane.showMessageDialog(scroll1,
					"Максимальное количество порций - 5", "Ошибка",
					JOptionPane.ERROR_MESSAGE);
		} else {
			index6 = index6 + 1;
	...
	private void plusCementActionPerformed(ActionEvent e) {
		if (index5 > 4) {
			JOptionPane.showMessageDialog(pan22,
					"Максимальное количество порций - 5", "Ошибка",
					JOptionPane.ERROR_MESSAGE);
		} else {
			index5 = index5 + 1;
	...
	private void muditemStateChanged(ItemEvent e) {
		if (mudnomud.isSelected()) {
			...
			Program.muds[index7 - 1][0] = Double
					.parseDouble(prodavka.getText());
	...
	DocumentListener mudprodW = new DocumentListener() {
		public void removeUpdate(DocumentEvent event) {
			if (mudnomud.isSelected()) {
				Program.muds[index7 - 1][1] = Double
						.parseDouble(mudW.getText());
				mudsSample.fireTableDataChanged();
			}
		}
	...
	DocumentListener mudprodV = new DocumentListener() {
		public void removeUpdate(DocumentEvent event) {
			if (mudnomud.isSelected()) {
				Program.muds[index7 - 1][2] = Double
						.parseDouble(mudV.getText());
				mudsSample.fireTableDataChanged();
			}
		}
	...
	DocumentListener mudprodY = new DocumentListener() {
		public void removeUpdate(DocumentEvent event) {
			if (mudnomud.isSelected()) {
				Program.muds[index7 - 1][3] = Double
						.parseDouble(mudY.getText());
				mudsSample.fireTableDataChanged();
			}
		}
	...
	JMenu calcMenu = new JMenu("Расчеты");
	JMenuItem modelItem = new JMenuItem("Моделирование");
	...
	// Надписи
	String labs[] = { "Буровой раствор", "Плотность, кг/м3",
			"Пластическая вязкость, Па*с",
			"Динамическое напряжение сдвига, Па", "Буферные жидкости",
			"Тампонажные растворы", "Продавочная жидкость" };
	...
	// переключатель
	JCheckBox mudnomud = new JCheckBox(
			"Использовать в качестве продавочной жидкости буровой раствор");
	
	Cement mix = new Cement();
	...
	public MainWindow() {
		...
		previousTable
				.setPreferredScrollableViewportSize(new Dimension(580, 70));
		...
		if (ret == JFileChooser.APPROVE_OPTION){
			...
			try{
				...
				if (line1.equals("dataofoilorgaswell")){
					...
					j=0; i = i+1;
					while (!FromFile.get(i).equals("mud_weight")){
						String oneline[] = FromFile.get(i).split(" ");
					...
					index4 = index4-1;
					//Записываем плотность бурового раствора
					i++;
					mudW.setText(FromFile.get(i));
					//Записываем вязкость бурового раствора
					i=i+2;
					mudV.setText(FromFile.get(i));
					//Записывем ДНС бурового раствора
					i=i+2;
					mudY.setText(FromFile.get(i));
					//Записываем буферные жидкости
					j=0; i=i+2;
					while (!FromFile.get(i).equals("cements")){
						String oneline[] = FromFile.get(i).split(" ");
						Program.spacers[j][0] = Double.parseDouble(oneline[0]);
						Program.spacers[j][1] = Double.parseDouble(oneline[1]);
						Program.spacers[j][2] = Double.parseDouble(oneline[2]);
						Program.spacers[j][3] = Double.parseDouble(oneline[3]);
						index6++;
						spacersSample.fireTableStructureChanged();
						i++; j++;
					}
					index6--;
					//Записываем цементные растворы
					j=0; i = i+1;
					while (!FromFile.get(i).equals("muds")){
						String oneline[] = FromFile.get(i).split(" ");
						Program.cements[j][0] = Double.parseDouble(oneline[0]);
						Program.cements[j][1] = Double.parseDouble(oneline[1]);
						Program.cements[j][2] = Double.parseDouble(oneline[2]);
						Program.cements[j][3] = Double.parseDouble(oneline[3]);
						Program.cements[j][4] = Double.parseDouble(oneline[4]);
						Program.cements[j][5] = Double.parseDouble(oneline[5]);
						index5++;
						cementsSample.fireTableStructureChanged();
						i++; j++;
					}
					index5--;
					//Записываем продавочную жидкость
					j=0; int i1=i+1;
					while (!FromFile.get(i1).equals("end")){
						String oneline[] = FromFile.get(i1).split(" ");
						i1++;
					}
					if (FromFile.get(i1-1).equals("yes")){
						mudnomud.setSelected(true);
					}
					else{
						i++;
						while (i<i1-1){
							String oneline[] = FromFile.get(i).split(" ");
							Program.muds[j][0] = Double.parseDouble(oneline[0]);
							Program.muds[j][1] = Double.parseDouble(oneline[1]);
							Program.muds[j][2] = Double.parseDouble(oneline[2]);
							Program.muds[j][3] = Double.parseDouble(oneline[3]);
							index7++;
							mudsSample.fireTableStructureChanged();
							i++; j++;
						}
	...
	private void saveFile() {
		...
		if (ret == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				buff.write("previous");
				buff.newLine();
				// Получаем последний значимый элемент массива предыдущей
				// колонны
				...
				buff.write("casing");
				buff.newLine();
				// Получаем последний значимый элемент массива цементируемой
				// колонны
